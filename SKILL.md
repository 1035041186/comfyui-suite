...
> **`--image`/`--video` 的取值（只两种，职责分明）**：
> - **`-`**——从 stdin 读 **base64**（`data:image/...;base64,` / `base64:` / 裸 base64）。
>   这是**跨 agent 的通用通道**：任何视觉 agent 的粘贴图都以 base64 表示，管道喂入即可，
>   **不落盘、无命令行 argv 上限**（大图用 `--image -` 而非内联）。解码后做**图片合法性校验**。
> - **本地文件路径**——用户**上传**、**显式给路径**、或 agent **一步直接拿到**的文件（无查找/拼路径）。
>   其余一律按文件路径处理（不再有内联 base64 / `@文件`，避免 路径/base64 歧义）。
>
> **选择规则**：当 base64 与文件路径都有、且文件路径能**一步直接拿到**（无需查找/构造）时，**优先文件路径**；
> 否则（如粘贴图、需查会话/附件才能定位路径）用 **`-`（stdin base64）**。
