...
| 取值 | 说明 |
|---|---|
| **本地文件路径** | 用户上传/显式给路径/已拿到的文件，如 `--image in.png` |
| **`-`** | 从**标准输入**读 **base64**（`data:image/...;base64,` 或裸 base64），跨平台、不落盘、无命令行长度限制 |

> 小提示：`-` 适合"手里只有 base64、没有文件"的场景（如粘贴到会话的图片）；有现成文件路径时
> 直接用路径更省事。脚本会对 base64 做图片格式校验，识别不出就报错。
