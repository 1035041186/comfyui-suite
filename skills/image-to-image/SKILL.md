...
1. **确认输入图**：必须是 agent 可访问的本地路径；URL 先下载；或经 stdin 传 base64
   （`--image -`，跨 agent / 不落盘 / 无 argv 上限）。
   向用户确认
   "保留什么、改变什么"。
...
3. **确定 denoise（重绘幅度）**——图生图最关键参数：
   ...
   > 当输入图是**上一轮生成的输出**（而非用户原始素材）时，本步即"**基于上一张再改**"的
   > 迭代调整——这种"不满意再调"的多轮调整走 `image-refine`（编排层），
   > 完整的"少动/大动"判定与"症状→杠杆"决策表见 `references/refine.md`、
   > 精炼版见 `skills/image-refine/SKILL.md`。迭代时建议**固化 seed + 只变 denoise/提示词**以便 A/B 对比。
...
| 参数 | 说明 |
|---|---|
| `--image` | 输入图片：本地文件路径 或 `-`（stdin 读 base64）（**必填**） |
