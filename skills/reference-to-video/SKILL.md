...
1. **确认两份素材**：参考图与参考视频都必须是本地路径；参考图也可给 base64
   （`data:image/...;base64,` 或 `base64:` 前缀，脚本自动解码上传、**不落盘**）。向用户确认
...
5. **交付**：按 `references/reporting.md` 的交付报告规范汇报——说清**调用序列**（用了哪些子 skill、顺序）
   与调用链（类型→工作流→模型）、关键参数（frames 尺寸 lora）与实际命令，`downloaded` 字段即结果路径。

## 参数要点

| 参数 | 说明 |
|---|---|
| `--image` | 角色/风格参考图：本地路径 或 base64（`data:...base64,`/`base64:` 前缀）（**必填**） |
| `--video` | 动作/运镜参考视频（**必填**） |
